...
const EventCalendar = ({ date }: { date: number }) => {
  ...
  return (
    <div className="flex flex-col font-sans bg-[#F5F5F5] text-center rounded-sm">
      ...
      <div className="bg-cacdRed p-1 text-[0.5rem] text-white uppercase rounded-b-sm">
        {month}
      </div>
    </div>
  );
...
const EventDetails = ({ event }: { event: Event }) => {
  return (
    <div className="p-6 flex flex-col justify-between">
      ...
      <div>
        <button className="border-2 border-cacdRed rounded-full mt-8 px-6 py-2 text-sm text-cacdRed font-bold">
